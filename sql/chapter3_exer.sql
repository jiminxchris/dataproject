...
-- 1-4. 원하는 열만 조회하기
SELECT chick_no, hatchday, egg_weight FROM chick_info;

-- 1-5. 열 이름 바꿔 조회하기
SELECT chick_no as cn, hatchday “부화일자” FROM chick_info;



-- 1-6. 데이터 정렬하기(ORDER BY)
SELECT chick_no, hatchday, egg_weight FROM chick_info ORDER BY egg_weight;

-- 1-6. 데이터 정렬하기(ORDER BY)
SELECT chick_no, hatchday, egg_weight FROM chick_info ORDER BY egg_weight DESC;
...
-- 1-8. 중복된 결과 제거하기(DISTINCT) hatchday
SELECT DISTINCT(egg_weight) FROM chick_info;

select COUNT(DISTINCT(egg_weight)) FROM chick_info;
...
SELECT * FROM chick_info WHERE egg_weight >= 70;
