...
-- ② RIGHT OUTER JOIN (결과는 INNER JOIN과 동일)
SELECT
a.chick_no, a.pass_fail, a.raw_weight,
b.order_no, b.customer
FROM
fms.prod_result a 
RIGHT OUTER JOIN fms.ship_result b
ON a.chick_no = b.chick_no;

-- ② FULL OUTER JOIN (결과는 INNER JOIN과 동일)
SELECT
a.chick_no, a.pass_fail, a.raw_weight,
b.order_no, b.customer
FROM
fms.prod_result a 
FULL OUTER JOIN fms.ship_result b
ON a.chick_no = b.chick_no;
...
FROM fms.master_code m, fms.chick_info a
...
JOIN fms.master_code m
  ON b.breeds = m.code AND m.column_nm = 'breeds'
WHERE
  a.pass_fail = 'P'
GROUP BY
  a.prod_date, m.code_desc;
...
AS pivot_rr(hatchday date, "Male" int, "Female" int);
